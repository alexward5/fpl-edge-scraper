...
async function processPlayerSeasonStats(playerMetadata, season) {
    ...
    const playerSeasonStats = await csvToJSON(`./Fantasy-Premier-League/data/${season}/cleaned_players.csv`);
    ...
    await seedTable(season, tableName, dbTableTemplates[tableName], playerSeasonStatsString);
}
